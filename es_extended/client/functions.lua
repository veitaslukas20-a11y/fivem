...
function ESX.ShowNotification(message, notifyType, length)
    return exports['1x-hud']:sendNotification({
        ...
        title = 'Pranešimas',
        message = message,
        duration = length and (length + 2000) or 6000,
        icon = 'info'
    })
end
...
function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
    if msg then 
        msg = msg:gsub("~r~", "<a style=color:red;>")
        msg = msg:gsub("~b~", "<a style='color:rgb(0, 213, 255);'>")
        msg = msg:gsub("~g~", "<a style='color:rgb(0, 255, 68);'>")
        msg = msg:gsub("~y~", "<a style=color:yellow;>")
        msg = msg:gsub("~p~", "<a style='color:rgb(220, 0, 255);'>")
        msg = msg:gsub("~f~", "<a style=color:grey;>")
        msg = msg:gsub("~m~", "<a style=color:darkgrey;>")
        msg = msg:gsub("~u~", "<a style=color:black;>")
        msg = msg:gsub("~o~", "<a style=color:gold;>")
        msg = msg:gsub("~s~", "</a>")
        msg = msg:gsub("~w~", "</a>")
        msg = msg:gsub("~n~", "<br>")
        msg = msg:gsub("\n", "<br>")
        msg = msg:gsub("~INPUT_CONTEXT~", "<a class='keybing'>E</a>")
        msg = msg:gsub("~INPUT_DETONATE~", "<a class='keybing'>G</a>")
        msg = msg:gsub("~INPUT_VEH_EXIT~", "<a class='keybing'>F</a>")
        msg = msg:gsub("~INPUT_RELOAD~", "<a class='keybing'>R</a>")
    end
...
function ESX.Game.SpawnVehicle(vehicleModel, coords, heading, cb, networked)
    ...
    CreateThread(function()
        ...
        SetTimeout(200, function()
			TriggerServerEvent("x99-vehstance:server:sync", GetVehicleNumberPlateText(vehicle), VehToNet(vehicle))
		end)
    end)
end
