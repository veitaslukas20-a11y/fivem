function OpenBossMenu()
    ...
    ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
        if isBoss then
            ...
            -- Add menu elements based on options
            ...
            ESX.OpenContext("right", elements, function(menu, element)
                ...
                elseif element.value == "withdraw_society_money" then
                    local withdrawElements = {
                        ...
                        {icon = "fas fa-check", title = TranslateCap('confirm'), value = "confirm_withdraw"},
                        {icon = "fas fa-arrow-left", title = TranslateCap('return'), value = "return"}
                    }
                    ESX.RefreshContext(withdrawElements)

                elseif element.value == "confirm_withdraw" then
                ...
                elseif element.value == "deposit_money" then
                    local depositElements = {
                        ...
                        {icon = "fas fa-check", title = TranslateCap('confirm'), value = "confirm_deposit"},
                        {icon = "fas fa-arrow-left", title = TranslateCap('return'), value = "return"}
                    }
                    ESX.RefreshContext(depositElements)

                elseif element.value == "confirm_deposit" then
                    local amount = tonumber(menu.eles[2].inputValue)
                    if amount == nil then
                        ESX.ShowNotification(TranslateCap('invalid_amount'))
                    else
                ...
                elseif element.value == "wash_money" then
                    local washElements = {
                        ...
                        {icon = "fas fa-check", title = TranslateCap('confirm'), value = "confirm_wash"},
                        {icon = "fas fa-arrow-left", title = TranslateCap('return'), value = "return"}
                    }
                    ESX.RefreshContext(washElements)

                elseif element.value == "confirm_wash" then
                    local amount = tonumber(menu.eles[2].inputValue)
                    if amount == nil then
                        ESX.ShowNotification(TranslateCap('invalid_amount'))
                    else
