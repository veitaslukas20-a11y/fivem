...
function SetIntoVehicle(vehicle, door)
    ...
    if freeSeat then
        ...
        TriggerServerEvent('s1m1s-neofgang:setinveh', id, NetworkGetNetworkIdFromEntity(vehicle), freeSeat)
    end
end
...
function SetOutVehicle(vehicle)
    ...
    if GetVehicleDoorLockStatus(vehicle) ~= 2 then
        ...
        if ped then
            ...
            id = GetPlayerServerId(id)
            TriggerServerEvent('s1m1s-neofgang:outveh', id, NetworkGetNetworkIdFromEntity(vehicle))
        end
    end
end

RegisterNetEvent('s1m1s-neofgang:setinveh', function(vehicle, seat)
    TaskWarpPedIntoVehicle(cache.ped, NetworkGetEntityFromNetworkId(vehicle), seat)
end)

RegisterNetEvent('s1m1s-neofgang:outveh', function(vehicle)
