...
function qbcore_switcher(type, number)
    ...
    elseif type == "moles_1" then
        if not IsPedModel(PlayerPedId(), 'mp_m_freemode_01') and not IsPedModel(PlayerPedId(), 'mp_f_freemode_01') then return end
        tempSkinTable['moles'].item = tonumber(number)
    elseif type == "moles_2" then
        if not IsPedModel(PlayerPedId(), 'mp_m_freemode_01') and not IsPedModel(PlayerPedId(), 'mp_f_freemode_01') then return end
        tempSkinTable['moles'].texture = tonumber(number)
    ...
    elseif type == "eye_squint" then
        tempSkinTable['eye_opening'].item = number
...
function updateValue(data)
    ...
    if IsPedModel(PlayerPedId(), 'mp_m_freemode_01') or IsPedModel(PlayerPedId(), 'mp_f_freemode_01') then
        ...
        SetPedHeadOverlay(myPed, 9, tempSkinTable['moles'].item, (tempSkinTable['moles'].texture / 10) + 0.0)
        ...
        SetPedFaceFeature(myPed, 11, (tempSkinTable['eye_opening'].item / 10) + 0.0)
