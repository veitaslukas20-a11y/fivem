...
RegisterNetEvent('s1m1s-mechanicjob:onFixkit', function()
	if IsPedInAnyVehicle(cache.ped, false) then 
		exports['1x-hud']:sendNotification({
			type = 'ERROR',
			title = 'Mechanikai',
			message = 'Deja, automobilio negalite tvarkyti sėdėdami jame.',
			duration = 6000,
			icon = 'wrench'
		})
		return 
	...
	if vehicle and IsEntityAVehicle(vehicle) and IsEntityVisible(vehicle) then
		if DoesEntityExist(vehicle) then
			...
			if success then
				...
				local result = lib.callback.await('s1m1s-mechanicjob:removeFixkit', false)
				if result then
...
function ImpoundVehicle(vehicle)
	...
	if status then
		TriggerServerEvent('s1m1s-mechanic:deleteveh', NetworkGetNetworkIdFromEntity(vehicle))
	end
end
			...
			if input then
				...
				if reason and amount then
					...
					--[[exports['1x-hud']:sendNotification({
						type = 'SUCCESS',
						title = 'Mechanikai',
						message = 'Sėkmingai išrašėte sąskaitą už '..amount..'€ su priežastimi '..reason..'.',
						duration = 5000,
						icon = 'wrench'
					})	]]				
				else 
					exports['1x-hud']:sendNotification({
						type = 'ERROR',
						title = 'Mechanikai',
						message = 'Įvedėte neteisingą sumą arba neteisingą priežastį.',
						duration = 5000,
						icon = 'wrench'
					})					
				end
	...
	if IsMechanic() and OnDuty then
		if data and data.amount and SPlayer then
			...
			--[[exports['1x-hud']:sendNotification({
				type = 'SUCCESS',
				title = 'Mechanikai',
				message = 'Sėkmingai išrašėte sąskaitą už '..data.amount..'€ su priežastimi '..data.reason..'.',
				duration = 5000,
				icon = 'wrench'
			})	]]
			SPlayer = nil
