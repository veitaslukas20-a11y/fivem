function OpenMenu()
    ...
    if data.legendGift then
        if data.legendGift.state then
            options[#options + 1] = {
                title = 'Legendos dovana',
                description = "Neatsiimta",
                icon = "gift",
                onSelect = function()
                    local result = lib.callback.await("peurost_vip:ClaimLegendGift", false)
                    if not result.state then
                        lib.notify({
                            title = 'VIP',
                            description = result.message,
